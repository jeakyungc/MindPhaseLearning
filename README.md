...
Static Svelte 5 codebase visualizer for `jeakyungc/MindPhase`. It analyzes `src` and `src-tauri/src`, writes a source graph plus source snapshot to JSON, and renders flow boards with cross-flow step nodes, function/import edges, an internal file tree, full-file source view, line range focus, and searchable symbol neighborhoods.
...
Validation checks that generated files, embedded source lines, file tree nodes, symbols, imports, and edges have consistent IDs, exact source line ranges, and local path metadata. `npm run check` runs Svelte diagnostics, and `npm run build` writes the static site to `build/`.
...
The generated code graph and source snapshot are committed under `static/generated/mindphase-codebase-graph.json` so Pages deploys do not need to clone and analyze MindPhase on every build.
...
- Active step function edges with internal source file and line range focus when available
- Static file tree for the analyzed MindPhase checkout
- Full-file source viewer with selected flow step and symbol line ranges highlighted
